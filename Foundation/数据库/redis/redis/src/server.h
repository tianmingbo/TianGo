...
#define ACTIVE_EXPIRE_CYCLE_LOOKUPS_PER_LOOP 20 /* 每个数据库检查的键数量 */
...
#define OBJ_ENCODING_RAW 0     /* 长度大于44字节的字符串, robj和sdshdr结构保存在两块内存中*/
#define OBJ_ENCODING_INT 1     /* 将数值型字符串转换成整型 */
...
#define OBJ_ENCODING_EMBSTR 8  /* 压缩字符串,robj和sdshdr结构保存在一块连续的内存中 */
...
typedef struct zskiplistNode {
    ...
    struct zskiplistNode *backward; //指向前驱节点,一个节点只有第一层有前驱节点指针
    struct zskiplistLevel {
        struct zskiplistNode *forward; //指向本层后驱节点
        unsigned long span; //本层后驱节点跨越了多少个第一层节点,用于计算节点索引值
    } level[];
} zskiplistNode;

typedef struct zskiplist {
    struct zskiplistNode *header, *tail; //指向头尾节点的指针
    unsigned long length; //节点数量
    int level; //skiplist最大的层数,最多为32层
} zskiplist;
...
struct redisServer {
    ...
    char runid[CONFIG_RUN_ID_SIZE + 1];  /* server 运行id,每次都不同 */
    ...
    long long stat_expiredkeys;     /* 过期key数量 */
    ...
    int aof_rewrite_incremental_fsync;/* 使用增量fsync? 只有重写产生的新内容会被fsync */
    int rdb_save_incremental_fsync;   /* fsync incrementally while rdb saving? */
    int aof_last_write_status;      /* C_OK or C_ERR */
    int aof_last_write_errno;       /* Valid if aof_last_write_status is ERR */
    int aof_load_truncated;         /* Don't stop on unexpected AOF EOF. */
    int aof_use_rdb_preamble;       /* aof使用rdb的文件头?,包括一些元数据 */
    ...
    /* Replication (slave) */
    char *masterauth;               /* 使用此密码与 master 进行 AUTH */
    ...
    int slave_announce_port;        /* 从库把自己的端口给主库 */
    ...
    int lfu_decay_time;             /* lfu衰减频率,默认为1 */
