...
class Map:
    """
    Map类用于存储所有的URL规则和一些配置参数。其中一些配置值仅存储在Map实例上，
    因为它们影响所有规则，而其他配置值只是默认值，可以为每个规则进行覆盖。

    参数：
        rules：URL规则的序列。
        default_subdomain：没有定义子域的规则的默认子域。
        charset：URL的字符集，默认为"utf-8"。
        strict_slashes：如果规则以斜杠结尾，但匹配的URL没有斜杠，会重定向到带有斜杠的URL。
        merge_slashes：在匹配或构建URL时合并连续的斜杠。匹配会重定向到规范化的URL。变量部分中的斜杠不会被合并。
        redirect_defaults：如果没有以默认的方式访问默认规则，会重定向到默认规则。这有助于创建唯一的URL。
        converters：一个添加额外转换器到转换器列表的字典。如果重新定义了一个转换器，它会覆盖原来的转换器。
        sort_parameters：如果设置为True，URL参数将被排序。详见url_encode函数的说明。
        sort_key：url_encode函数的排序键函数。
        encoding_errors：用于解码的错误方法。
