...
class ExecutionEngine:
    def __init__(self, crawler, spider_closed_callback: Callable) -> None:
        ...
        self.running = False
        self.paused = False
        self.scheduler_cls = self._get_scheduler_class(crawler.settings)  # Scheduler调度器
        downloader_cls = load_object(self.settings["DOWNLOADER"])  # download下载器
    ...
    def _download(self, request: Request, spider: Optional[Spider]) -> Deferred:
        ...
        dwld.addCallbacks(_on_success)  # 注册成功回调
