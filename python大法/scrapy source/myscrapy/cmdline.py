...
def execute(argv=None, settings=None):
    ...
    inproject = inside_project()  # 执行环境是否在项目中 主要检查scrapy.cfg配置文件是否存在
    cmds = _get_commands_dict(settings, inproject)  # 读取commands文件夹 把所有的命令类转换为{cmd_name: cmd_instance}的字典
    cmdname = _pop_command_name(argv)  # 从命令行解析出执行的是哪个命令
    if not cmdname:
