...
def load_object(path):
    """加载给定绝对对象路径的对象，并返回它。
     ...
     该对象可以是类、函数、变量或类的导入路径
     实例，例如 'scrapy.downloadermiddlewares.redirect.RedirectMiddleware'。

     如果 ``path`` 不是一个字符串，而是一个可调用的对象，例如类或
     一个函数，然后按原样返回它。
    """
...
def create_instance(objcls, settings, crawler, *args, **kwargs):
    ...
    if crawler and hasattr(objcls, "from_crawler"):  # 如果此中间件类定义了from_crawler 则调用此方法实例化
