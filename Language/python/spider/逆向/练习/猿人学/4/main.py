...
def get_hide_md5(data):
    ...
    need_hide_img = hex_md5.hexdigest()
    return need_hide_img


def revert(data):
    need_hide_key = get_hide_md5(data)
    groups = re.findall('<td>(.*?)</td>', data['info'])
    for group in groups:
        key_position.clear()
        tmp = ['', '', '', '']
        img_base_list = re.findall('<img src="(.*?)"\sclass="img_number\s(.*?)"\sstyle="left:(.*?)px"', group)
        img_base_list = [i for i in img_base_list if i[1] != need_hide_key]
        for index, info in enumerate(img_base_list):
            base_value, flag, position = info
            tmp[int(index + float(position) / 11.5)] = base_to_num_mapping[base_value]
...
def get_page_info():
    ...
    for i in range(1, 6):
        res = requests.get(f'{url}page={i}', headers={'User-Agent': 'yuanrenxue.project'}).json()
        revert(res)
