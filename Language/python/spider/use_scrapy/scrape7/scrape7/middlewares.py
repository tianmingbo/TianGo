...
class AuthorizationMiddleware:
    ...
    添加登录信息
    """
    account_pool_url = 'http://127.0.0.1:6789/antispider7/random'
    logger = logging.getLogger('middlewares.authorization')

    async def process_request(self, request, spider):
        async with aiohttp.ClientSession() as client:
            response = await client.get(self.account_pool_url)
...
class ProxyMiddleware(object):
    ...
    async def process_request(self, request, spider):
        async with aiohttp.ClientSession() as client:
            response = await client.get(self.proxy_url)
            if not response.status == 200:
                return
...
class Scrape7DownloaderMiddleware:
    # Not all methods need to be defined. If a method is not defined,
