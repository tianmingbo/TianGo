...
def main():
    while len(scraped_titles) < TOTAL_NUMBER:
        ...
        for i in range(len(elements)):
            element_title = elements[i].offspring(f'{PACKAGE_NAME}:id/tv_title')
            try:
            ...
            if title in scraped_titles:
                continue
            _, element_y = elements[i].get_position()
            if element_y > 0.8:
                scroll_up()
            time.sleep(1)
            element_data = scrape_detail(elements[i])
