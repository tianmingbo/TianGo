...
clusterNode *createClusterNode(char *nodename, int flags) {
    ...
    if (nodename)
        ...
        //没有指定目标节点的的name,则先随机生成一个,后续通信后,再更新name
...
int clusterStartHandshake(char *ip, int port, int cport) {
    ...
    /* 创建一个节点实例,并添加到cluster.node实例字典中 */
...
/* 负责处理clusterMsgDataGossip内容,当某个节点收到其他节点的消息时,如果消息的随机节点实例中包含该节点不认识的节点,
 * 则会将这些节点实例添加到实例字典中
 * */
        ...
        if (link->node) {
            ...
            if (nodeInHandshake(link->node)) {
                ...
                serverLog(LL_DEBUG, "Handshake with node %.40s completed.",
                          link->node->name);
                link->node->flags &= ~CLUSTER_NODE_HANDSHAKE; //清除握手标志,标志握手完成
...
void clusterSendPing(clusterLink *link, int type) {
    ...
    /*freshnodes、wanted 和 maxiterations控制clusterSendPing 函数具体会写入多少个节点的信息*/
    int freshnodes = dictSize(server.cluster->nodes) - 2;
    /*
     * wanted 变量的值和 freshnodes 大小也有关，wanted 的默认值是集群节点数的 1/10，
     * 但是如果这个默认值小于 3，那么 wanted 就等于 3。如果这个默认值大于 freshnodes，
     * 那么 wanted 就等于 freshnodes 的大小
     * */
    ...
    if (link->node && type == CLUSTERMSG_TYPE_PING)
        link->node->ping_sent = mstime(); //如果发送的是ping消息,则记录ping发送的时间
    clusterBuildMessageHdr(hdr, type); //构建ping消息头
...
void clusterCron(void) {
    ...
    dictEntry *de;
    int update_state = 0;
    int orphaned_masters; /* 没有从节点的主节点 */
    int max_slaves; /* Max number of ok slaves for a single master. */
    int this_slaves; /* Number of ok slaves for our master (if we are slave). */
    mstime_t min_pong = 0, now = mstime();
    clusterNode *min_pong_node = NULL;
    static unsigned long long iteration = 0; //静态局部变量,函数执行完后,该变量不会消失,负责统计cron执行的次数
    mstime_t handshake_timeout;

    iteration++; /* Number of times this function was called so far. */

    /* 希望将 self->ip 与 cluster-announce-ip 选项同步。
     * 该选项可以在运行时通过 CONFIG SET 设置，定期检查该选项是否更改以将其同步到 myself->ip
     */
    {
    ...
    /* 遍历实例字典中的实例,如果其中某个节点连接信息为空,则与该节点建立连接 */
    di = dictGetSafeIterator(server.cluster->nodes);
    server.cluster->stats_pfail_nodes = 0;
    while ((de = dictNext(di)) != NULL) {
        clusterNode *node = dictGetVal(de);
        ...
        /* 节点在调用 clusterSendPing 函数向其他节点发送 Ping 消息前，会检查它和其他节点连接情况，
         * 如果连接断开了，节点会重新建立连接*/
        if (node->link == NULL) {
            int fd;
            ...
            link->fd = fd;
            node->link = link;
            // 节点在和其他节点建立的连接上，设置的监听函数是 clusterReadHandler。
            // 所以，当一个节点收到 Ping 消息时，它就会在 clusterReadHandler 函数中进行处理
    ...
    if (!(iteration % 10)) {
        ...
        /* 随机选五个节点，然后在这五个节点中，
         * 选出最早向当前节点发送 Pong 消息的那个节点，并向它发送 Ping 消息 */
        for (j = 0; j < 5; j++) {
            de = dictGetRandomKey(server.cluster->nodes);
            clusterNode *this = dictGetVal(de);

            /* 过滤连接已断开,发送PING消息未收到回应,握手阶段的节点 */
...
void clusterCommand(client *c) {
    ...
    if (c->argc == 2 && !strcasecmp(c->argv[1]->ptr, "help")) {
        ...
        /* CLUSTER ADDSLOTS <slot> [slot] ... 添加slot*/
        /* CLUSTER DELSLOTS <slot> [slot] ... 移除slot*/
        int j, slot;
        ...
        /* CLUSTER REPLICATE <NODE ID> 建立主从关系*/
        clusterNode *n = clusterLookupNode(c->argv[2]->ptr);
 ...
 * 查询当前命令可以被哪个集群节点处理.
 *
 * 参数包括了节点收到的命令及参数。同时，它的参数中还包括了两个指针：hashslot 和 error_code，
 * 这两个指针分别表示命令访问的 key 所在的 slot（哈希槽），以及函数执行后的错误代码.
 *
 * */
clusterNode *getNodeByQuery(client *c, struct redisCommand *cmd,
                            robj **argv, int argc, int *hashslot, int *error_code) {
    clusterNode *n = NULL;
    robj *firstkey = NULL;
    int multiple_keys = 0;
    ...
    //针对收到的每个命令，逐一检查这些命令访问的 key 所在的 slots
    for (i = 0; i < ms->count; i++) {
        ...
        for (j = 0; j < numkeys; j++) {
            ...
            if (firstkey == NULL) {
                ...
                /* 查找的集群节点就是当前节点，而 key 所属的 slot 正在做数据迁出操作，
    ...
    /* M命令访问 key 所属的 slot 正在做数据迁出或迁入，
     * 而且当前命令就是用来执行数据迁移的 MIGRATE 命令，那么，getNodeByQuery 函数会返回当前节点*/
    if ((migrating_slot || importing_slot) && cmd->proc == migrateCommand)
        return myself;

    /* 命令访问 key 所属的 slot 正在做数据迁出，并且命令访问的 key 在当前节点数据库中缺失了，
     * missing_keys 大于 0。getNodeByQuery 函数会把 error_code 设置为 CLUSTER_REDIR_ASK，
     * 并返回数据迁出的目标节点 */
     ...
     * 此时，getNodeByQuery 函数会把 error_code 设置为 CLUSTER_REDIR_MOVED，
     * 并返回 key 所属 slot 对应的实际节点. */
    if (n != myself && error_code) *error_code = CLUSTER_REDIR_MOVED;
    return n;
}

/*
 * 根据 getNodeByQuery 函数返回的 error_code 的不同值，执行相应的代码分支，
 * 主要是把 key 所属 slot 对应集群节点的情况返回给客户端，从而让客户端根据返回的信息作出相应处理
 *
 * */
void clusterRedirectClient(client *c, clusterNode *n, int hashslot, int error_code) {
    if (error_code == CLUSTER_REDIR_CROSS_SLOT) {
        ...
        /* 当 error_code 被设置成 CLUSTER_REDIR_MOVED 时，返回 MOVED 命令，
         * 并把 key 所属的 slot、slot 实际所属的节点 IP 和端口号，返回给客户端；
         * 当 error_code 被设置成 CLUSTER_REDIR_ASK 时, 返回 ASK 命令，
