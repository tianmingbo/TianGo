...
SLAVEOF - 将服务器设置为其他服务器的从服务器。
    SLAVEOF host port - 设置主服务器地址和端口。
    SLAVEOF NO ONE - 清除设置,关闭复制。
REPLICAOF - 同SLAVEOF,设置从服务器。
ROLE - 查看主从角色状态。
REPLCONF - 在主服务器上配置从服务器参数。
    listening-port 端口 - 设置从端口。
    ip-address IP地址 - 设置从IP。
    capa 功能标识 - 设置从功能。
    getack - 请求从ACK。
    ack 偏移量 - 从回复ACK。
SYNC/PSYNC - 从服务器执行全量/部分重同步。
INFO - 查看复制信息。
PUBLISH - 向从服务器的频道发布消息。
CLIENT SETNAME - 设置主从连接的名称。
SENTINEL - 实现主服务器故障自动切换。

此外,还可以通过配置文件指定复制参数,如主从密码、磁盘less加载等。
这些命令覆盖了主从复制的方方面面,可以实现完整的主从管理。
 */
#include "server.h"
#include "cluster.h"
...
void replconfCommand(client *c) {
    ...
    for (j = 1; j < c->argc; j += 2) {
        //strcasecmp字符串比较，大小写不敏感
        if (!strcasecmp(c->argv[j]->ptr, "listening-port")) {
            long port;
            //listening-port 选项,设置从服务器的监听端口号
            if ((getLongFromObjectOrReply(c, c->argv[j + 1],
                                          &port, NULL) != C_OK))
                return;
            c->slave_listening_port = port;
        } else if (!strcasecmp(c->argv[j]->ptr, "ip-address")) {
            // ip-address 选项,设置从服务器的IP地址
...
/*
 * 实现了当Redis实例从从服务器切换成主服务器时,复制ID的更改
 * */
void shiftReplicationId(void) {
    //把replid复制给replid2
    memcpy(server.replid2, server.replid, sizeof(server.replid));
    /*把second_replid_offset设置为master_repl_offset+1，是因为当从服务器设置为主服务器后，
     * 可以接受从服务器的psync请求。比如原来已同步50Byte，那么从服务器会从51Byte开始请求*/
...
/* 向主库发送同步命令。 在 SYNC 之前发送 AUTH 和 REPLCONF 命令。
 * 该命令返回表示操作结果的 sds 字符串。 出错时第一个字节是“-”。
 */
...
/*主库收到命令后,会根据从库发送的主库ID和复制偏移量,来判断是全量复制还是增量复制,或是返回错误*/
int slaveTryPartialResynchronization(int fd, int read_reply) {
    ...
    if (!strncmp(reply, "+FULLRESYNC", 11)) {
        ...
        /* 将全量同步，丢弃cached_master。 */
        replicationDiscardCachedMaster();
        sdsfree(reply);
        return PSYNC_FULLRESYNC;
    }
    //主库返回+CONTINUE, 表示增量复制
...
void syncWithMaster(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    /* 在用户将实例转为主实例且 SLAVEOF NO ONE 后触发此事件 */
    ...
    /* 检查socket状态
     * fd: 要获取错误的socket描述符。
     * SOL_SOCKET: 表示操作的协议层为Socket层。
     * SO_ERROR: 选项名,表示获取socket的错误状态码。
     * &sockerr: 输出缓冲区指针,用于保存获取到的错误码。
     * &errlen: 输入/输出参数,指定了sockerr缓冲区的大小
     * */
    ...
    if (server.repl_state == REPL_STATE_CONNECTING) {
        ...
        //设置当前装填为REPL_STATE_RECEIVE_PONG
    ...
    /* 设置slave端口，以便Master的INFO命令能够正确列出slave监听端口 */
    ...
    /* 如果没有设置slave-announce-ip 选项，则跳过REPLCONF ip-address。 */
    if (server.repl_state == REPL_STATE_SEND_IP &&
        server.slave_announce_ip == NULL) {
        server.repl_state = REPL_STATE_SEND_CAPA;
    }

    /* 设置从库ip，以便在端口转发或者NAT的情况下，Master的INFO命令能够正确列出从机IP地址端口。 */
    if (server.repl_state == REPL_STATE_SEND_IP) {
    ...
    /* 告知master我们（slave）的能力。
    *
    * EOF：支持 EOF 式 RDB 传输以进行无盘复制。
    ...
    * 主机将忽略它不理解的功能。 */
    ...
    /* 从库状态变为REPL_STATE_SEND_PSYNC后,开始向主库发送PSYNC命令 */
    ...
    /* 如果master出现暂时性错误，我们应该稍后尝试从头开始PSYNC，从而转到错误路径。
     * 当服务器正在加载数据集或未与其主服务器连接等时，会发生这种情况。 */
    ...
    if (psync_result == PSYNC_CONTINUE) {
        //如果是PSYNC_CONTINUE, 从syncWithMaster返回,后续执行增量复制
...
int connectWithMaster(void) {
    ...
    //在建立的连接上注册读写事件, 回调函数为syncWithMaster
    ...
    //完成连接后,将状态设置为REPL_STATE_CONNECTING
...
/* 与主服务器同步时中止批量数据集的异步下载。
 * 切勿直接调用此函数，而应使用cancelReplicationHandshake()。
 */
...
void replicaofCommand(client *c) {
    ...
    /* REPLICAOF NO ONE 取消从服务器的配置，slave不干了 */
...
void replicationCron(void) {
    ...
    /* 如果从库实例的状态是REPL_STATE_CONNECT,那么开始和主库建立连接 */
